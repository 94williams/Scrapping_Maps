...
def desplazamiento_infinito():
    try:
        ...
        while True:
            ...
            time.sleep(3)
            Pizzerías = driver.find_elements(By.CSS_SELECTOR, ".Nv2PK")

            if len(Pizzerías) == prev_count:
                break

            prev_count = len(Pizzerías)
    except Exception as e:
        print(f"Error durante el desplazamiento: {e}")

# Iterar sobre cada ubicación
for ubicacion in ubicaciones:
    print(f"\nBuscando Pizzería en {ubicacion['nombre']}...")
    url = f"https://www.google.com/maps/search/Pizzería/@{ubicacion['lat']},{ubicacion['lng']},14z"
    driver.get(url)
    time.sleep(10)  # Esperar carga inicial
    desplazamiento_infinito()

    Pizzerías = driver.find_elements(By.CSS_SELECTOR, ".Nv2PK")
    print(f"{len(Pizzerías)} Pizzerías encontradas en {ubicacion['nombre']}.")

    for Pizzería in Pizzerías:
        try:
            nombre = Pizzería.find_element(By.CSS_SELECTOR, ".qBF1Pd").text
            direccion = Pizzería.find_element(By.CSS_SELECTOR, ".W4Efsd").text

            # Extraer sitio web (si está disponible)
            try:
                sitio_web_element = Pizzería.find_element(By.CSS_SELECTOR, "a[href^='https://']")
            ...
            print(f"Error extrayendo datos de una Pizzería: {e}")
...
if data:
    ...
    nombre_archivo = f"Pizzerías_CDMX_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
