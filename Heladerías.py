...
def desplazamiento_infinito():
    try:
        ...
        while True:
            ...
            time.sleep(3)
            Heladerías = driver.find_elements(By.CSS_SELECTOR, ".Nv2PK")

            if len(Heladerías) == prev_count:
                break

            prev_count = len(Heladerías)
    except Exception as e:
        print(f"Error durante el desplazamiento: {e}")

# Iterar sobre cada ubicación
for ubicacion in ubicaciones:
    print(f"\nBuscando Heladerías en {ubicacion['nombre']}...")
    url = f"https://www.google.com/maps/search/Heladerías/@{ubicacion['lat']},{ubicacion['lng']},14z"
    driver.get(url)
    time.sleep(10)  # Esperar carga inicial
    desplazamiento_infinito()

    Heladerías = driver.find_elements(By.CSS_SELECTOR, ".Nv2PK")
    print(f"{len(Heladerías)} Heladerías encontradas en {ubicacion['nombre']}.")

    for Heladería in Heladerías:
        try:
            nombre = Heladería.find_element(By.CSS_SELECTOR, ".qBF1Pd").text
            direccion = Heladería.find_element(By.CSS_SELECTOR, ".W4Efsd").text

            # Extraer sitio web (si está disponible)
            try:
                sitio_web_element = Heladería.find_element(By.CSS_SELECTOR, "a[href^='https://']")
            ...
            print(f"Error extrayendo datos de una Heladería: {e}")
...
if data:
    ...
    nombre_archivo = f"Heladería_CDMX_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
